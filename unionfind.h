...
class UnionFind3
{
  ...
  private:
    ...
    string drivingProblem(int L, int W, vector<vector<double>> &p)
    {
        ...
        for (int i = 0; i < n; i++)
            for (int j = i + 1; j < n; j++)
                if (ds(p[i][0], p[i][1], p[j][0], p[j][1]) <= 6.0)
                    uf.Union(i, j);
        for (int i = 0; i < n; i++)
        {
            if (1.0 * W - p[i][1] <= 5.0)
        ...
        return uf.Find(n) == uf.Find(n + 1) ? "no" : "yes";
    }
  ...
  private:
    ...
    {
      public:
        UnionFind(int n) : _id(n), _w(n, 1)
        {
            iota(begin(_id), end(_id), 0);
        }
